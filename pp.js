...
var pola2 = [document.getElementsByClassName("pola11"), document.getElementsByClassName("pola22"), document.getElementsByClassName("pola33")];
var pola = document.getElementsByClassName("pola");
var jambg = document.getElementsByClassName("jamgacor");
var jambg2 = document.getElementsByClassName("pola123");

var random = ["âœ… âŒ âŒ", "âŒ âœ… âŒ", "âŒ âŒ âœ…", "âœ… âŒ âœ…", "âŒ âœ… âœ…", "âŒ âŒ âŒ", "âœ… âœ… âœ…", "âœ… âœ… âŒ"];
var random2 = [20, 50, 10, 10, 30, 70, 50, 10, 30, 100, 50, 70, 30, 10, 20, 20, 30, 50, 10, 50, 70];
var random3 = ["Auto", "Auto", "Manual 25", "Manual 20", "Manual 15", "Manual 15", "Auto", "Auto", "Auto", "Auto", "Auto", "Auto"];
var game = window.location.pathname;

if (game == "/" || game == "/index.html") {
    game = "";
}
var xxx = 0;
...
for (let i = 0; i < text.length; i++) {
    ...
    if (xx < 20) {
        ...
        body2.classList.add("green1");
    } else {
        bar.classList.add("yellow");
        body2.classList.add("yellow");
    }
    var ppola = para[i];
    var pppola = pola[i];
    if (xx < 20) {
        ppola.style.backgroundColor = "#b32400";
    } else if (xx > 50) {
        ppola.style.backgroundColor = "#6B8E23";
    } else {
        ppola.style.backgroundColor = "#cc8800";
    }
    if (xx >= 20) {
        var hour2 = (hour + 7) % 24;
        var target = (xx % 3) + 1;
        ...
        for (let j = 0; j < 3; j++) {
            if (random3[p3[j]] == "Auto") {
                pola1[j][i].innerText = random[p2[j]];
                pola2[j][i].innerText = random2[p2[j]] + " " + random3[p3[j]];
            } else {
                // console.log(pola1[j][i]);
                pola1[j][i].innerText = random[p[j]];
                pola2[j][i].innerText = random3[p3[j]];
            }
        }

        if (xx > 70) {
            pola[i].style.backgroundColor = "";
            jambg[i].style.backgroundColor = "";
        } else {
            pola[i].style.backgroundColor = "";
            jambg[i].style.backgroundColor = "";
        }
        ...
        jam[i].innerText = "coba nanti !";
        pola[i].style.backgroundColor = "#33333d";
        jambg[i].style.color = "#33333d";
        pola1[0][i].innerHTML = "PILIH GAME LAIN !!";
        pola1[1][i].innerHTML = "PILIH GAME LAIN !!";
        pola1[2][i].innerHTML = "PILIH GAME LAIN !!";
    }
}

// Cari elemen dengan kelas "pola1"
const pol = document.querySelector('.pola2');

// Periksa apakah elemen memiliki elemen p dengan teks "PILIH GAME LAIN !!"
if (pol.querySelector('p')?.textContent === 'PILIH GAME LAIN !!') {
    // Jika ya, cari elemen dengan kelas "jamgacor1"
    const jamgacor = document.querySelector('.jamgacor1');

    // Ubah warna teks elemen menjadi merah
    jamgacor.style.color = 'red';

    // Ubah warna teks elemen p menjadi biru
    pol.style.color = 'blue';
}
